...
$adminEmail = $_SESSION['reset_email'] ?? '';
    ...
    <div class="row">
      <div class="col-lg-6">
        <div class="d-lg-flex align-items-center justify-content-center bg-light-300 d-lg-block d-none flex-wrap vh-100 overflowy-auto bg-01">
          <div>
            <img src="assets/img/authentication/authentication-07.svg" alt="Img">
          </div>
        </div>
      </div>
            ...
            <form id="forgotPasswordForm" method="post" novalidate>
              ...
              <div class="card">
                <div class="card-body p-4">
                  ...
                  <div id="resetMessage" class="text-center mt-2 text-danger"></div>
              ...
              <div class="mt-5 text-center">
                <p class="mb-0">Copyright &copy; <?= date('Y') ?>  - <?= htmlspecialchars($_SESSION['societe_nom']) ?></p>
              </div>
            </form>
          </div>
        </div>
...
document.getElementById('forgotPasswordForm').addEventListener('submit', async function (e) {
  ...
  try {
    const res = await fetch('../../controller/adminController.php', {
      method: 'POST',
      body: formData
    });
    ...
    const msgElem = document.getElementById('resetMessage');
    msgElem.textContent = result.message || 'No message received from server.';
    msgElem.style.setProperty('color', (result.status === 'success') ? 'green' : 'red', 'important');

    // ✅ Redirect if the code was sent successfully
    if (result.status === 'success') {
      setTimeout(() => {
        window.location.href = 'verify-code.php';
      }, 1500); // Wait 1.5 seconds before redirecting (optional)
    }
